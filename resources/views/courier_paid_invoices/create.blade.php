...
@section('content')
<h4 class="mb-3">Add Courier Paid Invoice</h4>

<div class="card shadow-sm p-4">
    <form method="POST" action="{{ route('courier_paid_invoices.store') }}">
        @csrf
        ...
        <div class="mb-3">
            ...
            <select name="courier_name" id="courier_name" class="form-control">
                @foreach(config('constants.couriers') as $code => $name)
                    <option value="{{ $name }}" {{ old('courier_name') == $name ? 'selected' : '' }}>
                        {{ $name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="invoice_type" 
                ...
                @foreach(config('constants.paid_invoice_types') as $code => $type)
                    <option value="{{ $type }}" {{ old('invoice_type') == $type ? 'selected' : '' }}>
                        {{ ucfirst($type) }}
                    </option>
                @endforeach
            </select>
